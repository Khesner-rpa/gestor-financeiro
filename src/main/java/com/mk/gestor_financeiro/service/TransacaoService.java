...
public class TransacaoService {
    ...
    public DashboardDados carregarDashboard(String emailUsuario) {
        ...
        return new DashboardDados(
                resumo,
                calcularCategorias(transacoesDoMes),
    ...
    private List<CategoriaResumo> calcularCategorias(List<Transacao> transacoesDoMes) {
        ...
        return despesasPorCategoria.entrySet().stream()
                ...
                .map(item -> new CategoriaResumo(
                        ...
                        item.getValue(),
                        calcularPercentual(item.getValue(), totalDespesas)
                ))
                .toList();
    }

    private MetaFinanceira calcularMeta(DashboardResumo resumo) {
        BigDecimal objetivo = resumo.despesasMes().multiply(BigDecimal.valueOf(3));

        if (objetivo.compareTo(BigDecimal.valueOf(1000)) < 0) {
            objetivo = BigDecimal.valueOf(1000);
        }

        BigDecimal acumulado = resumo.saldoAtual().max(ZERO);
